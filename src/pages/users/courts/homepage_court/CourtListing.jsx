...
const CourtListing = () => {
  ...
  useEffect(() => {
    if (urlType && URL_TO_TYPE[urlType] && URL_TO_TYPE[urlType] !== courtType) {
    ...
    const fetchCourts = async () => {
      try {
        ...
        if (response.isSuccessed) {
          const courtsData = (response.resultObj?.items || []).map((court) => ({
            id: court.id,
            name: court.name,
            location: court.sportsComplexModelView?.address || "Bình Triệu",
            image: court.imageUrls?.[0] || loginImg,
            rating: court.ratingSummaryModelView?.averageRating || 4.5,
            totalRatings: court.ratingSummaryModelView?.totalReviewerCount || 0,
          }));
      ...
      <div className="container">
                ...
                <div>
                  ...
                  <div className="action-row">
                    <button
                      ...
                      onClick={() => handleCourtClick(featuredCourt.id)}
                    >
                      Đặt sân
                    </button>
                    <div className="rating">
                      <FaStar className="star-icon" />
                      <span>{featuredCourt.rating.toFixed(1)} đánh giá</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="sport-tabs">
          ...
          <button
            ...
            style={{
              ...
              fontWeight: isActiveType(SPORT_TYPES.SOCCER) ? "bold" : "normal",
            }}
          >
            Bóng đá
          </button>
        ...
        <div className="nearby-courts-card">
          ...
          {!loading && !error && nearbyCourts.length === 0 && (
            <div className="empty-state">Không tìm thấy sân nào gần đây</div>
          )}
                ...
                {nearbyCourts.map((court) => (
                  ...
                  >
                    <div className="court-image-container">
                      ...
                      />
                      <div className="location-badge">{court.location}</div>
                    </div>
                    <div className="court-card-footer">
                      <div className="court-rating">
                        <FaStar className="star-icon-small" />
                        <span>
                          {court.rating.toFixed(1)} ({court.totalRatings} đánh
                          giá)
                        </span>
                      </div>
